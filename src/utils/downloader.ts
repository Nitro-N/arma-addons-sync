...
export default class Downloader {
    public static download(fileUrl: string, dir: string): Promise<any> {
        return new Promise<any>((resolve, reject) => {
            ...
            const send = () => {
                ...
                getUri(fileUrl, options, (err: any, rs: stream.Readable) => {
                    if (err) {
                        reject(err);
                        Downloader.changeConnectionsCount(urlKey, -1);
                    } else {
    ...
    private static readonly maxConnectionsCount = 10;
    private static readonly retryTimeout = 5000;
    private static connectionsCount = new Map<string, number>();

    private static changeConnectionsCount(key: string, diff: number) {
